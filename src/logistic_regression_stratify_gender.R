...
datasets <- c("dataset_non_vs_all.xlsx",
              "dataset_non_vs_all_female.xlsx",
              "dataset_non_vs_all_male.xlsx",
              "dataset_pos_vs_neg.xlsx",
              "dataset_pos_vs_neg_female.xlsx", 
              "dataset_pos_vs_neg_male.xlsx")
...
for (dataset_ in datasets) {
  ...
  # Stratify by sex
  sexes <- unique(df_data$dem_sex)
  
  for (sex in sexes) {
    df_sex <- subset(df_data, dem_sex == sex)
    
    # Formula for the model
    formula <- as.formula(paste(target, "~", paste(features, collapse = "+")))
    
    # Run logistic regression
    model <- glm(formula, data = df_sex, family = binomial)
    
    # Summary of the model
    model_summary <- capture.output(summary(model))
    writeLines(model_summary, paste0(path_output_summary))
    ...
    odds_plot <- ggplot(odds_ratios, aes(x = odds_ratios, y = feature)) +
      ...
      theme_minimal() +
      labs(y = "", x = "Odds Ratio") +
      ggtitle(paste("Odds Ratios and Confidence Intervals for Features (Sex:", sex, ")"))
    
    ggsave(paste0(path_output_odds_plot,".png"), plot = odds_plot)
    
    # Save the results to Excel
    write.xlsx(odds_ratios, file = paste0(path_output_odds,".xlsx"))
  }
}
